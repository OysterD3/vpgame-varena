...
const { prefix, vpgameSecret } = require("./config");
...
client.on("message", msg => {
  ...
  if (command === "foo") {
    ...
    if (type === "player" || type === "gameplay") {
      if (
        ...
        const embedMsg = new RichEmbed()
          ...
          .setTimestamp();
        client.channels.get("556664792715100164").send(embedMsg);
        msg.reply(
      ...
      if (isDigitOnly(steamID) && !attachmentNotExist) {
        ...
        const embedMsg = new RichEmbed()
          ...
          .attachFiles(attachmentArr)
          .setTimestamp();
        client.channels.get("556664843172577320").send(embedMsg);
        msg.reply(
          "your report has been recorded. On duty admin will take necessary action ASAP"
        );
        msg.delete();
    ...
    const embedMsg = new RichEmbed()
      .setTitle("VPGAME Discord Bot Commands")
      .addField(
